...
#ifndef _MAZE_H_
#define _MAZE_H_
#define RAG_Div_2 0.00872664625997164788461845384244
#define RAG_Div 0.01745329251994329576923690768489
...
public:
	...
	void	Draw_View(const float,const double m[]);

	// check which cell we need to draw
	void	Draw_Cell(const double pos[], double left[], double right[]);

	// find the nearest edge, and return its index
	drawCoordinate		Find_Edge(double xs, double ys, double xe, double ye, bool Clip);

	// check which part of wall need to draw
	// return edge[0]:left, edge[1]:right
	void	Draw_Part(double edge[][2], const double pos[], double left[], double right[], bool isTrans);

	void	Draw_Point(const double dot[2],const bool L);
